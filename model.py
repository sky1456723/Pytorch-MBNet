...
class wavMBNet(nn.Module):
    def __init__(self, num_judges, activation = nn.ReLU):
        super(wavMBNet, self).__init__()

        self.mean_net_rnn = nn.LSTM(input_size = 768, hidden_size = 128, num_layers = 1, batch_first = True, bidirectional = True)
        ...
        self.bias_net_rnn = nn.LSTM(input_size = 768, hidden_size = 64, num_layers = 1, batch_first = True, bidirectional = True)
        ...
        self.judge_embedding = nn.Embedding(num_embeddings = num_judges, embedding_dim = 768)
    def forward(self, spectrum, judge_id):
        #spectrum should have shape (batch, time, 768)
        batch = spectrum.shape[0]
        time = spectrum.shape[1]
        mean_feat, (h, c) = self.mean_net_rnn(spectrum)
